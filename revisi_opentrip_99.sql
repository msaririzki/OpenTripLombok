...
(11, 'DGN001', 'Andong', 1726, 19, 1, 'Nama Gunung Andong berasal dari nama sebuah daun yang sering disebut “Andong“. Daun Andong dalam kepercayaan Jawa merupakan salah satu daun yang dipersyaratkan dalam ritual. Khususnya dalam acara Selamatan. Daun Andong memiliki makna “Andongoo” yaitu Berd', 'Ngablak,Tlogrojo'),
(12, 'DGN002', 'anjasmoro', 2282, 5, 1, 'Gunung Anjasmoro merupakan sebuah gunung yang terdapat di pulau Jawa, Indonesia. Ketinggian gunung ini ialah 2.282 meter. Gunung Anjasmoro termasuk ke dalam wilayah Kabupaten Jombang, Kabupaten Mojokerto, dan Kota Batu, Jawa Timur. Gunung Anjasmoro terlet', 'jomlang, mojokerto'),
(13, 'DGN003', 'Argopuro', 3088, 5, 1, 'Gunung Argapura memiliki ketinggian 3.088 mdpl. Gunung di timur Jawa ini menyimpan keindahan alam yang luar biasa. Satu hal lain yang membuat banyak pendaki penasaran dengan gunung ini adalah panjangnya jalur perjalanan yang harus ditempuh hingga puncak e', 'Probolinggo,lumajang'),
(14, 'DGN004', 'Arjuno', 3339, 5, 1, 'Gunung Arjuno (terkadang dieja Gunung Arjuna) adalah sebuah gunung berapi kerucut (istirahat) di Jawa Timur, Indonesia dengan ketinggian 3.339 m dpl. Gunung Arjuno secara administratif terletak di perbatasan Kota Batu, Kabupaten Malang, dan Kabupaten Pasu', 'Malang,Pasuruan'),
(15, 'DGN005', 'Batok', 2440, 5, 1, 'Gunung Batok adalah sebuah gunung yang terletak di Jawa Timur, Indonesia. Gunung ini memiliki ketinggian 2.440 meter di atas permukaan laut dan berada di antara empat wilayah kabupaten, yakni Kabupaten Probolinggo, Kabupaten Pasuruan, Kabupaten Lumajang, ', 'Probolinggo, lumajang'),
(16, 'DGN006', 'Bromo', 2329, 5, 1, 'Gunung Bromo (dari bahasa Sanskerta: Brahma, salah seorang Dewa Utama dalam agama Hindu) atau dalam bahasa Tengger dieja &quot;Brama&quot;, adalah sebuah gunung berapi aktif di Jawa Timur, Indonesia. Gunung ini memiliki ketinggian 2.329 meter di atas perm', 'Probolinggo, Pasuruan'),
(17, 'DGN007', 'Butak', 2868, 5, 1, 'Gunung Butak adalah gunung stratovolcano yang terletak di Kabupaten Malang, Jawa Timur, Indonesia. Gunung Butak terletak berdekatan dengan Gunung Kawi. Tidak diketemukan catatan sejarah atas erupsi dari Gunung Butak sampai saat ini.[1] Gunung ini berada p', 'Malang'),
(18, 'DGN008', 'Ciremai', 3078, 5, 1, 'Gunung Ceremai (sering kali secara salah kaprah dinamakan &quot;Ciremai&quot;) (Aksara Sunda Baku: ᮌᮥᮔᮥᮀ ᮎᮨᮛᮨᮙᮦ, Latin: Gunung Ceremé) adalah gunung berapi kerucut yang secara administratif termasuk dalam wilayah dua kabupaten, yakni Kabupaten Kuningan da', 'Majalengka, Kuningan'),
(19, 'DGN009', 'Gede', 2958, 5, 1, 'Gunung Gede (Aksara Sunda Baku: ᮌᮥᮔᮥᮀ ᮌᮨᮓᮦ, Gunung Gedé) merupakan sebuah gunung api bertipe stratovolcano yang berada di Pulau Jawa, Indonesia. Gunung Gede berada dalam ruang lingkup Taman Nasional Gede Pangrango, yang merupakan salah satu dari lima tama', 'Cianjur, Sukabumi'),
(20, 'DGN010', 'Pangrango', 3019, 5, 1, 'Gunung Pangrango (Aksara Sunda Baku: ᮌᮥᮔᮥᮀ ᮕᮍᮢᮍᮧ) merupakan sebuah gunung yang terdapat di pulau Jawa, Indonesia. Gunung Pangrango mempunyai ketinggian setinggi 3.019 meter dari permukaan laut. Puncaknya dinamakan Puncak Mandalawangi. Puncak Mandalawangi ', 'Cianjur, Sukabumi');
...
INSERT INTO `kategori_gunung` (`id_kategori`, `nama_kategori`) VALUES
(1, 'Kategori 1');
-- --------------------------------------------------------
...
REFERENCES usermenu(id) ON DELETE CASCADE;

ALTER TABLE gallerygunung
ADD CONSTRAINT fk_gallerygunung_kode_gunung FOREIGN KEY (kode_gunung)
REFERENCES gunung(kode_gunung) ON DELETE CASCADE;
...
REFERENCES kategori_gunung(id_kategori) ON DELETE CASCADE;
...
REFERENCES paket_pendakian(kodepaket) ON DELETE CASCADE;
...
ALTER TABLE pembayaran
ADD CONSTRAINT fk_pembayaran_email FOREIGN KEY (email)
REFERENCES user(email) ON DELETE CASCADE;

ALTER TABLE paket_pendakian
ADD CONSTRAINT fk_paket_pendakian_email FOREIGN KEY (email)
REFERENCES user(email) ON DELETE CASCADE;

ALTER TABLE pemesanan
ADD CONSTRAINT fk_pemesanan_email FOREIGN KEY (email)
REFERENCES user(email) ON DELETE CASCADE;

ALTER TABLE pembayaran
ADD CONSTRAINT fk_pembayaran_kode_pemesanan FOREIGN KEY (kode_pemesanan)
REFERENCES pemesanan(kode_pemesanan) ON DELETE CASCADE;
